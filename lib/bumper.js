...
class Bumper {
  ...
  bump () {
    if (this.config) {
      ...
      console.log('this.config is undefined')
    }

    if (this.vcs) {
      console.log('this.vcs: ', this.vcs)
    } else {
      console.log('this.vcs is undefined')
    }

    if (this.ci) {
      console.log('this.ci: ', this.ci)
    }  else {
      console.log('this.ci is undefined')
    }
    ...
    // if ((__.get(this.config, 'computed.ci.isPr')) &&
    //     (__.get(this.config, 'computed.ci.branch') === 'master')){
    //   logger.log('Not a merge build, skipping bump')
    //   return Promise.resolve()
    // }

    // bail if you are:
    //  Not a merge build or
    //  Not a branch on the main repo that is not a pre-release branch.



    // Two issues:
    // push a branch to main repo - pr-bumper does a github release
    // greenkeeper creates a branch on the main repo - pr-bumper does a github release
    //
    // when isPr === false and branch != master then this is an upstream branch and not
    // a pre-release branch that would be added to with PRs
    //
    // if branch === /greenkeeper/ and isPr === false ---> bail out
    // if branch
    //
    let branchName = String(__.get(this.config, 'computed.ci.branch'))
    console.log('branchName: ', branchName, ' typeof:', typeof branchName)

    if ((__.get(this.config, 'computed.ci.isPr') === false) &&
        (branchName.match('greenkeeper\/') !== null)) {
      logger.log('Not a merge build, skipping bump on upstream branch')
      // throw new Cancel(`Skipping bump on upstream branch.`)
      return Promise.resolve()
    }
